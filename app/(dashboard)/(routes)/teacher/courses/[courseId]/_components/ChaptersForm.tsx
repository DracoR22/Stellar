...
const ChaptersForm = ({initialData, courseId}: ChaptersFormProps) => {
 ...
 const onEdit = (id: string) => {
  ...
  return (
    <div className="mt-6 border bg-purple-100 rounded-md p-4 relative">
        {isUpdating && (
            <div className="absolute h-full w-full bg-purple-500/20 top-0 right-0 rounded-md flex items-center justify-center">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 mt-4">
            <FormField control={form.control} name="title" render={({ field }) => (
              ...
              <Button isLoading={isSubmitting} disabled={!isValid} type="submit" variant="primary">
                Create
              </Button>
          </form>
        </Form>
